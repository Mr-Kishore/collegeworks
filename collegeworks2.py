...
def main(audio_file, text_file, video_file):
    ...
    # Change: Reshape the processed audio tensor to the required dimensions for RNN
    reshaped_audio = torch.tensor(processed_audio).view(1, -1, 1)  # <-- Added this line to reshape the audio

    # Change: Pass reshaped audio tensor into RNN
    audio_features = RNNFeatureExtractor(input_size=1, hidden_size=64)(reshaped_audio)  # <-- Use reshaped audio tensor
    ...
    # Feature fusion
    # Change: Convert `audio_features` to a numpy array for fusion
    fused_features = fuse_features(audio_features.detach().numpy(), spatial_features, linguistic_features)  # <-- Convert to numpy
